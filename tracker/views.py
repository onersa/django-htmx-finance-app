...
def transactions_list(request):
    ...
    context = {
        ...
        "total_expenses": f"R {total_expenses:,.2f}",
        "net_income":f"R {net_income:,.2f}"
               }
...
def create_transaction(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            transaction.save()
            context = {"message": "Transaction was add successfully!"}
...
def delete_transaction(request, pk):
    ...
    transaction.delete()
    context = {"message": f"Transaction of R {transaction.amount} on {transaction.date} was deleted successfully"}
    return render(request, 'tracker/partials/transaction-success.html', context)
